...
**Goal:** Implement UFW application profile management with support for system profiles and custom profile creation.

**Architecture:** Leverage UFW's app profile system (/etc/ufw/applications.d/). Create custom profiles in vexo-custom file. Provide UI to list, apply, create, and manage profiles.

**Tech Stack:** Python, Rich, UFW CLI, INI-style config files

---

## Task 1: Implement Application Profiles Module

**Files:**
- Modify: `modules/firewall/profiles.py`

**Step 1: Replace profiles.py with full implementation**
...
```bash
git add modules/firewall/profiles.py
git commit -m "feat(firewall): implement application profiles with custom profile support"
```
...
1. **List Profiles works:**
   - Shows system and custom profiles
   - Shows applied status

2. **Apply Profile works:**
   - Can select and apply any profile
   - Shows profile info before applying

3. **Create Custom Profile works:**
   - Creates profile in vexo-custom file
   - Can apply immediately after creation

4. **Edit Custom Profile works:**
   - Can modify title, description, ports

5. **Delete Custom Profile works:**
   - Removes from file
   - Option to remove firewall rules

6. **Profile Info works:**
