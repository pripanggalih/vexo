# Phase 3: Edit Worker & Clone Worker

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add ability to edit existing worker configurations and clone workers with new names.

**Architecture:** Create edit.py module with functions to view and modify worker settings. Use parsed config from common.py to display current values and generate updated configs.

**Tech Stack:** Python, subprocess (existing)

**Prerequisite:** Complete Phase 2 (worker templates)

---

## Task 1: Create Edit Worker Module

**Files:**
- Create: `modules/supervisor/edit.py`
...
def _show_edit_options(worker_name):
    ...
    while True:
        ...
        console.print(f"[bold]Command:[/bold] {config['command'][:60]}...")
        ...
        choice = show_submenu(title=f"Edit {worker_name}", options=options)
        
        if choice == "numprocs":
            _edit_numprocs(worker_name, config, content)
        elif choice == "user":
            _edit_user(worker_name, config, content)
        elif choice == "queues":
            _edit_queues(worker_name, config, content)
        elif choice == "memory":
            _edit_memory(worker_name, config, content)
        elif choice == "connection":
            _edit_connection(worker_name, config, content)
        elif choice == "autostart":
            _toggle_autostart(worker_name, config, content)
        elif choice == "autorestart":
            _toggle_autorestart(worker_name, config, content)
        elif choice == "full":
            _edit_full_config(worker_name)
...
```bash
git add modules/supervisor/edit.py
git commit -m "feat(supervisor): add edit and clone worker functionality"
```
...
git commit -m "feat(supervisor): add worker management submenu with edit/clone"
```
...
After Phase 3, the supervisor module will have:

**Edit Worker:**
- Change number of processes
- Change user
- Change queue names (Laravel workers)
- Change memory limit (Laravel workers)
- Change connection (Laravel workers)
- Toggle autostart/autorestart
- Edit full config (advanced)

**Clone Worker:**
- Select source worker
- Enter new name
- Auto-update program name and log path
...
- `modules/supervisor/edit.py` (new)
