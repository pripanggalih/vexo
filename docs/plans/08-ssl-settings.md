...
**Goal:** Implement SSL settings management including default CA selection, alert configuration, auto-renewal settings, and monitoring schedule.

**Architecture:** Store settings in /etc/vexo/ssl/settings.json. Support email and webhook alerts. Integrate with certbot timer for auto-renewal.

**Tech Stack:** Python, Rich, JSON storage, systemd

---

## Task 1: Implement Settings Module

**Files:**
- Modify: `modules/ssl/settings.py`

**Step 1: Replace settings.py with full implementation**
...
**Step 2: Update common.py imports**

Add at the end of `modules/ssl/common.py`:
...
```bash
git add modules/ssl/settings.py modules/ssl/common.py
git commit -m "feat(ssl): implement settings with CA selection, alerts, and thresholds"
```
...
**Files:**
- Remove: `modules/ssl.py` (old file)

**Step 1: Remove old ssl.py and commit**

```bash
rm modules/ssl.py
git add -A
git commit -m "refactor(ssl): complete SSL module enhancement

Phase 1: Package refactor + dashboard
Phase 2: Issue certificate (single/SAN/wildcard)
Phase 3: Import custom certificates
Phase 4: Certificate management
Phase 5: DNS providers
...
1. **Default CA works:**
   - Can change default CA
   - Persists across sessions

2. **Alert Settings works:**
   - Enable/disable alerts
   - Configure email and webhook
   - Test alerts

3. **Auto-Renewal works:**
   - Check timer status
   - Enable/disable timer
   - Configure hooks
   - Dry-run test

4. **Alert Thresholds works:**
   - Change critical/warning/notice days

5. **View All Settings works:**
   - Displays all current settings

6. **Reset works:**
   - Resets to defaults with confirmation
