...
**Goal:** Implement certificate management functions: view details, manual renew, revoke, and delete certificates.

**Architecture:** Work with both certbot-managed and custom certificates. Use certbot commands for certbot certs, manual operations for custom certs.

**Tech Stack:** Python, Rich, Certbot CLI, OpenSSL CLI

---

## Task 1: Implement Certificate Management Module

**Files:**
- Modify: `modules/ssl/manage.py`

**Step 1: Replace manage.py with full implementation**
...
```bash
git add modules/ssl/manage.py
git commit -m "feat(ssl): implement certificate management (view/renew/revoke/delete)"
```
...
1. **View Details works:**
   - Shows all certificate info
   - Chain, fingerprint, HTTPS test

2. **Renew works:**
   - Certbot renewal for LE certs
   - Warning for custom certs
