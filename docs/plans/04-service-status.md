...
**Goal:** Add service monitoring with auto-detect systemd services, predefined vexo services, and service control actions.

**Architecture:** Create service.py module that auto-detects running systemd services and merges with predefined vexo-managed services. Provide service actions (start, stop, restart, reload, enable, disable) and log viewing.

**Tech Stack:** Python, subprocess (existing shell.py utilities)

**Prerequisite:** Complete Phase 1 (monitor package structure)

---

## Task 1: Create Service Module Base

**Files:**
- Create: `modules/monitor/service.py`
...
```bash
git add modules/monitor/service.py
git commit -m "feat(monitor): add service discovery with systemd"
```

---

## Task 2: Add Service List Views

**Files:**
- Modify: `modules/monitor/service.py`

**Step 1: Add service list views**

Add after `filter_vexo_services()`:
...
```bash
git add modules/monitor/service.py
git commit -m "feat(monitor): add service list views"
```

---

## Task 3: Add Service Actions

**Files:**
- Modify: `modules/monitor/service.py`

**Step 1: Add service actions function**

Add after `show_failed_services()`:
...
```bash
git add modules/monitor/service.py
git commit -m "feat(monitor): add service actions and log viewer"
```

---

## Task 4: Wire Up Service Menu
...
git commit -m "feat(monitor): wire up service status menu"
```
...
After Phase 4, the service status will have:

- **All Services:** View all systemd services with status
- **Vexo Services Only:** Filter to common VPS services (nginx, mysql, php-fpm, etc.)
- **Failed Services:** Quick view of problematic services
- **Service Actions:**
  - Start/Stop/Restart/Reload
  - Enable/Disable autostart
  - View logs (journalctl)
- **Auto-detection:** Automatically finds all systemd services
- **Pattern matching:** Supports wildcard patterns like `php*-fpm`

Files added/modified:
- `modules/monitor/service.py` (new)
