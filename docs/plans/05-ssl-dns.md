...
**Goal:** Implement DNS provider configuration for DNS-01 challenge, supporting Cloudflare, DigitalOcean, Route53, and manual mode.

**Architecture:** Store encrypted credentials in /etc/vexo/ssl/dns/. Use certbot DNS plugins. Provide API test functionality.

**Tech Stack:** Python, Rich, Certbot DNS plugins, Cloud provider APIs

---

## Task 1: Implement DNS Providers Module

**Files:**
- Modify: `modules/ssl/dns_providers.py`

**Step 1: Replace dns_providers.py with full implementation**
...
def get_configured_provider(domain=None):
    ...
    # In future, could map domains to specific providers
    for provider_key, provider in DNS_PROVIDERS.items():
        if os.path.exists(provider["credentials_file"]):
            return {
                "key": provider_key,
...
def _save_credentials(provider_key, credentials):
    ...
    try:
        if provider_key == "google":
            ...
            # Write credentials from template
...
def test_provider():
    ...
    # Read credentials and test
    show_info(f"Testing {provider['name']}...")
    
    # For a real test, we'd need to parse the credentials file
    # and make an API call. Simplified version:
...
```bash
git add modules/ssl/dns_providers.py
git commit -m "feat(ssl): implement DNS provider configuration for wildcard certs"
```
...
1. **List Providers works:**
   - Shows all providers with status
   - Indicates configured vs not configured

2. **Configure works:**
   - Installs certbot plugin if needed
   - Collects appropriate credentials
   - Tests API connection
   - Saves with secure permissions

3. **Test Provider works:**
   - Verifies credentials file exists
   - Checks plugin availability

4. **Remove Provider works:**
