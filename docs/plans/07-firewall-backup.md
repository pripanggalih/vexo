...
**Goal:** Implement comprehensive backup and restore functionality with named profiles, configuration comparison, and auto-backup settings.

**Architecture:** Store backups as JSON files in /etc/vexo/firewall/backups/. Each backup contains UFW rules, IP groups, rate limits, and app profiles. Support diff comparison between configurations.

**Tech Stack:** Python, Rich, JSON storage, UFW CLI
...
**Files:**
- Modify: `modules/firewall/backup.py`
...
def _restore_ufw_rules(rules, defaults):
    ...
    for rule in rules:
        ...
        # This is a simplified approach - complex rules may need more parsing
...
```bash
git add modules/firewall/backup.py
git commit -m "feat(firewall): implement backup/restore with compare and auto-backup"
```

---

## Task 2: Final Integration Commit

**Step 1: Commit all firewall enhancements**

```bash
git add -A
git status
git commit -m "feat(firewall): complete firewall module enhancement

Phase 1: Package refactor + status dashboard
Phase 2: Port management with presets
Phase 3: IP management with groups
Phase 4: Rate limiting
Phase 5: Application profiles
Phase 6: Logging & monitoring
Phase 7: Backup & restore

All features integrated and accessible from main menu."
```
...
1. **Create Backup works:**
   - Can create named backup
   - Selectable components
   - Saved to correct location

2. **Restore Backup works:**
   - Lists available backups
   - Shows backup details
   - Creates pre-restore backup
   - Restores rules correctly

3. **Compare Configs works:**
   - Can compare any two configs
   - Shows added/removed rules
   - Shows IP group differences

4. **Auto-Backup Settings works:**
   - Can toggle auto-backup
   - Can set retention count
   - Cleanup removes old backups

5. **Manage Backups works:**
   - Lists all backups
   - Can view details
   - Can rename/delete/export
