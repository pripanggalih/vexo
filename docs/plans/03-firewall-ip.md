# Phase 3: IP Management

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement comprehensive IP management with allow/deny rules, IP whitelist, and IP groups for easier management.

**Architecture:** Three flexibility levels - Simple (single IP), Advanced (CIDR, per-port), Full (IP groups). Groups stored in JSON config file.

**Tech Stack:** Python, Rich, InquirerPy, JSON storage, UFW CLI

---

## Task 1: Create IP Groups Storage

**Files:**
- Modify: `modules/firewall/common.py` (add IP group functions)

**Step 1: Add IP group management functions to common.py**
...
git commit -m "feat(firewall): add IP group storage functions"
```

---

## Task 2: Implement IP Management Module

**Files:**
- Modify: `modules/firewall/ip_management.py`

**Step 1: Replace ip_management.py with full implementation**
...
```bash
git add modules/firewall/ip_management.py
git commit -m "feat(firewall): implement IP management with whitelist and groups"
```
...
1. **Allow/Deny IP works:**
   - Simple mode (all ports)
   - Advanced mode (specific port/protocol)
   - Both deny and reject actions

2. **IP Whitelist works:**
   - Add/remove IPs
   - View whitelist
   - Apply rules to firewall

3. **IP Groups work:**
   - Create group with IPs
   - Edit group (add/remove IPs)
   - Delete group
   - Apply group rules

4. **List IP Rules works:**
   - Shows only IP-specific rules
