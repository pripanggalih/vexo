...
**Goal:** Implement configurable rate limiting for any port with preset templates and custom threshold support.

**Architecture:** Use UFW's built-in `limit` command for basic rate limiting. Store custom configurations in JSON. Support presets for common services.

**Tech Stack:** Python, Rich, UFW CLI, JSON storage

---

## Task 1: Add Rate Limit Storage Functions

**Files:**
- Modify: `modules/firewall/common.py`

**Step 1: Add rate limit functions to common.py**

Add after IP group functions:
...
```bash
git add modules/firewall/common.py
git commit -m "feat(firewall): add rate limit storage functions"
```

---

## Task 2: Implement Rate Limiting Module

**Files:**
- Modify: `modules/firewall/rate_limiting.py`

**Step 1: Replace rate_limiting.py with full implementation**
...
```bash
git add modules/firewall/rate_limiting.py
git commit -m "feat(firewall): implement rate limiting with presets and SSH protection"
```
...
1. **Enable Rate Limit works:**
   - Can select port and protocol
   - Preset selection works
   - Custom threshold option works
   - Rule applied to UFW

2. **Quick SSH Protection works:**
   - One-click SSH protection
   - Replaces allow with limit rule

3. **Configure Limits works:**
   - Can toggle enabled/disabled
   - Can change preset
   - Can delete configuration

4. **List Rate Limits works:**
   - Shows configured limits
   - Shows UFW limit rules

5. **Remove Rate Limit works:**
   - Can select and remove limit rule
   - Option to re-add as allow rule
