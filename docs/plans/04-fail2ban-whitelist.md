# Fail2ban Phase 4: Whitelist Management

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement comprehensive whitelist management with global/per-jail support, IP groups, and trusted sources auto-update.

**Architecture:** Store whitelist in JSON config, sync to fail2ban ignoreip directive. Support IP groups and auto-updating trusted sources like Cloudflare.

**Tech Stack:** Python, Rich (tables), JSON config, fail2ban ignoreip

---

## Task 1: Implement Whitelist Module

**Files:**
- Modify: `modules/fail2ban/whitelist.py`

**Step 1: Implement full whitelist.py**
...
from ui.menu import confirm_action, text_input, select_from_list, run_menu_loop, checkbox_select
...
def _apply_whitelist_to_fail2ban():
    ...
    try:
        ...
        # If still in DEFAULT and not updated
        if in_default and not ignoreip_updated:
            new_lines.append(f"{ignoreip_line}\n")
...
**Step 2: Commit whitelist module**

```bash
git add modules/fail2ban/whitelist.py
git commit -m "feat(fail2ban): implement whitelist with groups and trusted sources"
```
...
1. Whitelist features:
   - Global whitelist (IPs and CIDR ranges)
   - Per-jail whitelist overrides
   - IP groups for managing sets of IPs
   - Trusted sources (Cloudflare, etc.) with auto-update
   - Import/export functionality
   - Apply to fail2ban ignoreip

2. Data stored in `~/.vexo/fail2ban/`:
   - `whitelist.json` - All whitelist data
   - `trusted_sources.json` - Trusted source settings
